    ...
    public class Entities : GeneratorBase
    {
        ...
        private bool RequiresImplicitExtension(Entity entity)
        {
            var result =
                ...
                && entity.AggregateExtensionChildren.Concat(entity.AggregateExtensionOneToOnes)
                ...
                yield return new
                {
                    ...
                    PrimaryKey = new
                    {
                        ParentReference = !entity.IsAggregateRoot
                            ? new
                            {
                                IsAggregateExtension = entity.IsAggregateExtensionTopLevelEntity,
                                ...
                                ParentRecordInterfaceName = GetParentRecordInterfaceName(entity),
                                ...
                                RecordProperties = entity.ParentAssociation.PropertyMappings.Select(
                                    pm => new
                                    {
                                        CSharpType = pm.ThisProperty.PropertyType.ToCSharp(includeNullability: true),
                                        ClassName = entity.Name,
                                        pm.ThisProperty.PropertyName,
                                        ParentClassName = entity.ParentAssociation.OtherEntity.Name,
                        ...
                        NonParentProperties = entity.Identifier.Properties.Where(p => !p.IsFromParent)
                            .Select(
                                ...
                                {
                                    entity.IsAbstract,
                                    DisplayName = !entity.IsAbstract && UniqueIdSpecification.IsUSI(p.PropertyName)
                            ...
                            p => new
                            {
                                ...
                                Attributes = string.Join(
                                    ...
                                    (!p.PropertyType.IsNullable && !CSharpDefaultHasDomainMeaning(p)
                                        ? new[] {"RequiredWithNonDefault"}
                                        : new string[0]).Concat(
                                        p.PropertyType.ToCSharp() == "DateTime"
                                            ? new[] {"SqlServerDateTimeRange"}
                                            : new string[0])
                                    .Concat(
                                        p.PropertyType.ToCSharp() == "string"
                                            ? new[]
                                            {
                                                "StringLength(" + p.PropertyType.MaxLength + ")",
                                                "NoDangerousText"
                                            }
                                            : new string[0])
                                    .Concat(
                    ...
                    LookupProperties = entity.Properties.Where(p => p.IsLookup)
                        .OrderBy(p => p.PropertyName)
                        .Select(
                            p => new
                            {
        ...
        {
            return
                $"{Namespaces.Entities.NHibernate.RelativeNamespace}.{entity.Aggregate.Name}Aggregate.{entity.SchemaProperCaseName()}.";
        }
            ...
            return new
            {
                LookupProperty = p.IsLookup
                    ? new
                    {
                        ...
                        LookupEntityName = p.LookupEntity.Name,
                        p.PropertyType.IsNullable,
                        IdFieldName = "_" + p.PropertyName.ToCamelCase(),
                        ValueFieldName = "_" + p.GetLookupValuePropertyName().ToCamelCase(),
                        CSharpDeclaredType = p.PropertyType.ToCSharp(includeNullability: true),
                ...
                UsiProperty = UniqueIdSpecification.IsUSI(p.PropertyName)
                    ? new
                    {
                        ...
                        UniqueIdFieldName = "_" + p.PropertyName.ReplaceSuffix("USI", "UniqueId").ToCamelCase(),
                ...
                DelegatedProperty = IsDelegatedToBaseProperty(entity, p)
                    ? new {BasePropertyName = GetBasePropertyName(entity, p)}
                    : _notRendered
            };
        }
        ...
        private static string GetBasePropertyName(Entity entity, EntityProperty property)
        {
            return entity.IsDerived && property.IsIdentifying
                ? property.Entity.BaseAssociation.PropertyMappingByThisName[property.PropertyName].OtherProperty.PropertyName
                : null;
        }
        ...
        private string GetParentRecordInterfaceName(Entity entity)
        {
            ...
            var namespaceSegment = EdFiConventions.BuildNamespace(Namespaces.Entities.Records.RelativeNamespace, properCaseName);
